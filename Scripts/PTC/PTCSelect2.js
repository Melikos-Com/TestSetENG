(function ($) {
    ...
    const $pageSize = 10;          //頁面長度
    const $delayTime = 250;        //傳輸daley時間(ms)
    const $defaultPageIndex = 0;   //初始頁面數
    const $dataType = 'JSON';      //資料回傳格式
    const $transType = 'POST';     //傳輸格式
    const $cache = false;          //暫存
